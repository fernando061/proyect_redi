...
@user_controller_bp.route('/user/register', methods=['POST'])
...
@user_controller_bp.route('/user/login', methods=['POST'])
@use_args(login_args)
def login(args):
    try:
        ...
        userDict['roles'] = user_roles

        del userDict['_sa_instance_state']
        del userDict['password']
        del userDict['id']
        del userDict['updated_at']

        return {'success': True, 'message': 'Login successful', 'user':userDict,'token': token}, 200
    except ValueError as e:
        ...
        return response
@user_controller_bp.route('/user/refresh-token', methods=['GET'])
@jwt_required()
def refresh_token():
    current_user_id = get_jwt_identity()
    user = User.query.filter_by(id=current_user_id).first()
    token = UserService.refresh_token(user)
    userDict = user.__dict__.copy()
    user_roles = [role.name for role in user.roles]
    userDict['roles'] = user_roles
